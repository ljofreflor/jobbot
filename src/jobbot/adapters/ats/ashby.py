"""Ashby ATS adapter — open + known-field map only (HITL submit)."""
...
class AshbyAdapter:
    """Prefill helper for Ashby-hosted applications."""

    name = "ashby"

    def detect_method(self, job: JobPosting) -> ApplyMethod:
        url, kind = resolve_ats_url(job)
        if url and (kind == AtsKind.ASHBY or detect_ats(url) == AtsKind.ASHBY):
