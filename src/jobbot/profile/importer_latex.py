...
_SECTION_KINDS: dict[str, tuple[str, ...]] = {
    ...
    "skills": ("habilidades", "skills", "competencias", "técnicas", "tecnicas"),
...
@dataclass
class ImportResult:
    """Result of importing a legacy LaTeX CV."""
...
def write_generated_profile(result: ImportResult, destination: Path) -> Path:
    """Write imported profile YAML. Never writes to profile.yaml by itself."""
    destination.parent.mkdir(parents=True, exist_ok=True)
    header = (
        f"# Generated by jobbot profile import-latex\n"
...
def _section_kind(title: str) -> str | None:
    ...
    for kind, aliases in _SECTION_KINDS.items():
...
def _parse_summary(sections: list[tuple[str, str]]) -> str | None:
    for title, body in sections:
        if _section_kind(title) == "summary":
...
def _parse_experiences(
    ...
    for title, body in sections:
        if _section_kind(title) != "experience":
            continue
...
def _parse_education(
    ...
    seen_ids: set[str] = set()
    for title, body in sections:
        if _section_kind(title) != "education":
            continue
        for entry in _iter_cventries(body):
            ...
            edu_id = _unique_id(_slugify(f"{institution}-{degree}") or "education", seen_ids)
            item: dict[str, Any] = {
                "id": edu_id,
...
def _parse_skills(
    ...
    for title, body in sections:
        if _section_kind(title) != "skills":
            continue
        for label, value in _iter_cvitems(body):
            group = _map_skill_group(label)
...
def _parse_publications(
    ...
    seen_ids: set[str] = set()
    for title, body in sections:
        if _section_kind(title) != "publications":
            continue
        for year_raw, value in _iter_cvitems(body):
            ...
            pub_id = _unique_id(_slugify(pub_title) or f"pub-{year or 'x'}", seen_ids)
...
def _read_following_itemize(text: str) -> tuple[list[str], int]:
    ...
    if not match:
        return [], 0
    start = match.end()
    end_match = re.search(r"\\end\{itemize\}", text[start:])
    if not end_match:
        return [], 0
    inner = text[start : start + end_match.start()]
...
def _cventry_to_experience(
    ...
    dates = _parse_date_range(entry.dates)
    exp_id = _unique_id(_slugify(f"{company}-{title}") or "experience", seen_ids)
    ...
    for idx, bullet in enumerate(entry.bullets, start=1):
        ach_id = _unique_id(_slugify(f"{exp_id}-{bullet[:40]}") or f"{exp_id}-a{idx}", ach_seen)
...
def _parse_date_range(raw: str) -> dict[str, Any]:
    ...
    start = _parse_one_date(start_raw, end=False)
    current = False
    end: str | None = None
    ...
    if len(parts) == 1:
    ...
    elif end_raw == "" or re.search(
        r"presente|present|actual|hoy|current",
        end_raw,
        re.IGNORECASE,
    ):
        current = True
        ...
        end = _parse_one_date(end_raw, end=True)

    return {"start_date": start, "end_date": end, "current": current}


def _parse_one_date(raw: str, *, end: bool) -> str | None:
    text = raw.strip()
    if not text:
        return None
    ...
    if month_match:
        mon = _MONTHS[_month_key(month_match.group(1))]
...
def _map_skill_group(label: str) -> str:
    norm = _normalize_key(label)
    for key, group in _SKILL_GROUP_MAP.items():
