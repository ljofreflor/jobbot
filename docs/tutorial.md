...
JobBot is a local tool with one fact base and many views. This walks the loop once, then states the
practices that keep it usable and the guarantees the code actually enforces. Where a guarantee is
enforced by a test, the test is named: those are the ones that will fail if the guarantee breaks.
...
`data/profile.yaml` is the only source of professional truth. Importers write
...
## 4. Keep your permanent profiles current

```bash
uv run jobbot browser sessions                   # who is signed in, and on which port
uv run jobbot cv propagate                       # plan only
uv run jobbot cv propagate --targets getonboard --apply --cdp http://127.0.0.1:9224
```
...
uv run jobbot companies signup NOMBRE            # prepares the registration; you create it
...
uv run jobbot cv advise                          # small, concrete presentation improvements
```
...
**Nothing is submitted on your behalf.** JobBot opens forms and prefills the fields it knows, and
stops. It never presses Send on an email, never submits an application, and never creates an
account for you: registration is prepared and handed over.
